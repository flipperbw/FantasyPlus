    ...
    if (siteType === 'espn') {
        ...
        if (fix_css) {
            if (onFreeAgencyPage || onGeneralProjPage) {
                var trade_btns = jQuery('a').filter(function() { return jQuery(this).text() === 'Trade'; });
                trade_btns.css({
                ...
                var claim_btns = jQuery('a').filter(function() { return jQuery(this).text() === 'Claim'; });
                claim_btns.css({
...
function isActivityDataCurrent(pid, upd, typ, live) {
    ...
    if (typ === 'league') {
        ...
        dlog('update week for player: ' + pid + ': ' + update_week, 1);
    ...
    if (r === false) {
        dlog('Activity data out of date for player: ' + pid + ', type is: ' + typ + ', by: ' + ((current_time - upd) / 1000), 1);
    }
...
function addColumns() {
    ...
    if (siteType === "espn") {
        ...
        var findHeader = function(hname, pname, is_parent) {
            ...
            var seek_headers = player_table_header.find('th').filter(function() {
                var h_j = jQuery(this);
            ...
            if ((team_header > -1) && (player_name_header > -1)) {
                for (let i=1; i < parsed_proj.length; i++) {
                    ...
                    if (currentline.length < team_header + 1) {
                        dlog('Error fetching projections');
                        continue;
                    }
        ...
        if ((!player_cell_text) || (normavg === "--")) {
            ...
            if (!player_id) {
                ...
                if (isActivityDataCurrent(new_player_id, player_stored_activity_league_updated, 'league', live_game) && isActivityDataCurrent(new_player_id, player_stored_activity_games_updated, 'games')) {
                    dlog('Using cache for player activity: ' + new_player_id, 1);
                    ...
                    jQuery.ajax({
                        ...
                        if (!po) {
                            ...
                            for (let i=0; i < player_activity_pts.length; i++) {
                                ...
                                else if (player_cell_text.match(/(D\/ST|TQB|HC)$/) && jQuery.isNumeric(player_activity_pts[i])) {
                            ...
                            dlog('Player Activity pts', 1);
                            dlog(player_activity_pts, 1);
                            ...
                            // Only fetch on tuesday
                                ...
                                dlog('using cache for games played for: ' + pid, 1);
                                calcAdjAvg(currRow, pid, p_data['games_played'], p_data_league['weekly_points']);
                            }
                            else {
                    ...
                    jQuery.ajax({
                        ...
                        dlog('Could not get player info: ' + cust_player_href);
                        insertAdjAvg(currRow, null, null, [], []);
                    }).done(function(po) {
                        var cust_data = this.custom_data;
                        var cust_player_href = cust_data.p_href;
                        ...
                        if (!po) {
                            dlog('No data in player info: ' + cust_player_href);
                            insertAdjAvg(currRow, null, null, [], []);
                        }
                        else {
                            po = cleanHTML(po);
...
function calcAdjAvg(thisrow, player_id, games_played, weekly_points) {
    ...
    dlog('Past points data', 1);
    dlog(past_weekly_points_data, 1);
    dlog(games_played, 1);
...
function insertAdjAvg(thisrow, p_avg, p_med, games_played, weekly_points) {
    ...
    if (show_spark) {
        ...
        if (weekly_points && weekly_points.length > 0) {
            ...
            dlog('we expect for pos: ' + pos_name + ', to get: ' + expected_projection, 1);
...
function insertDepth(cell, depthData) {
    if (depthData.constructor !== Array || depthData.length < 3) {
        ...
        if (posname === 'D/ST') {
            ...
            if (p_depth === '--') {
                dlog('Could not get depth for player: ' + depthData[0]);
            }
            cell.text(p_depth);
            ...
            if (players_sorted.length) {
                ...
                dlog('No team data for pos: ' + posname);
                dlog(team_data);
            }
        }
    }
